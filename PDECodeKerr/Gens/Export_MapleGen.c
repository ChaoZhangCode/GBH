...
int main(int argc, char *argv[])
{
	...
	fprintf(poutfile,"FEvar := [x::numeric,y::numeric,r_H::numeric,M::numeric,");
	for (j = 0 ; j < FEn; ++j)
	{
	...
	fprintf(qoutfile,"ICvar := [x::numeric,y::numeric,r_H::numeric,M::numeric,delta::numeric]:\n\n");
	fprintf(qoutfile,"BCvar := [x::numeric,y::numeric,r_H::numeric,M::numeric,");
	for (j = 0 ; j < FEn; ++j)
	{
	...
	strcpy(vardecl,"(double xk, double yk, double r_H, double M,");
	for (j = 0 ; j < FEn; ++j)
	{
		for (k = 0; k < FEdlength; ++k)
		{
	...
	for (i = 0; i < FEn; ++i)
	{
		fprintf(coutfile,"double IC%iout (double xk, double yk, double r_H, double M, double delta);\n",i);
	}
	fprintf(coutfile,"\n\n");
	...
	for (bc = 0; bc < BClength; ++bc)
	{
		for (i = 0; i < FEn; ++i)
		{
			...
			fprintf(coutfile,"double BC%i%c%iout (double xk, double yk, double r_H, double M,",i,BCdim[bc],BCloc[bc]);
			for (j = 0 ; j < FEn; ++j)
			{
			...
			for (j = 0; j < FEn; ++j)
			{
				for (k = 0; k < BCdlength; ++k)
				{
					fprintf(coutfile,"double dBC%i%c%id%id%i%iout (double xk, double yk, double r_H, double M,",i,BCdim[bc],BCloc[bc],j,BCdx[k],BCdy[k]);
					for (l = 0 ; l < FEn; ++l)
					{
