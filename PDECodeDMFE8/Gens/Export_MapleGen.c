...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < FEn; ++i)
	{
		...
		for (j = 0; j < FEn; ++j)
		{
			...
			for (k = 0; k < FEdlength; ++k)
			{
			//	fprintf(poutfile,"dFE%id%id%i%ifull := collect(diff(FE%ifull,u%id%i%i),collectexport):\n",i,j,FEdx[k],FEdy[k],i,j,FEdx[k],FEdy[k]);
			fprintf(poutfile, "dFE%id%id%i%ifull := collect(simplify(diff(FE%ifull,u%id%i%i),size),collectexport):\n", i, j, FEdx[k], FEdy[k], i, j, FEdx[k], FEdy[k]);
			}
		}
		fprintf(poutfile,"\n");
	}
